...
public class PopulatorLakeAndCreek extends BlockPopulator {


	private static final int MIN_BLOCK_COUNT = 400;
...
//		}
	...
	private void buildAirAndWaterfall(final Set<Block> ground, final Set<Block> blocks, final Random random) {
		...
		final Map<Integer, List<Block>> groupByXZ = blocks.stream().collect(Collectors.groupingBy(block -> block.getX() * 16 + block.getZ()));
		final List<Block> aboveLayer = groupByXZ.keySet().stream().map(key -> groupByXZ.get(key).stream().max((blockA, blockB) -> Integer.compare(blockA.getY(), blockB.getY()))).filter(Optional::isPresent).map(Optional::get).map(block -> block.getRelative(BlockFace.UP)).collect(Collectors.toList());

		aboveLayer.forEach(block -> {
			if (TREE_MATERIALS.contains(block.getType())) {
				treeBlocksToCheck.add(block);
			}
		});
	...
	private void handleTreeRemoval(final List<Block> treeBlocksToCheck) {
		...
		while (!treeBlocksToCheck.isEmpty()) {
			...
			final boolean tooLarge = findConnectedTree(treeBlocksToCheck.get(0), connectedTree);

			if (!tooLarge && !checkTreeOnSolidGround(connectedTree)) {
	...
	private boolean findConnectedTree(final Block block, final Set<Block> result) {
		result.add(block);
		...
		for (final BlockFace blockFace : FACES_TO_CHECK) {
			...
			if (TREE_MATERIALS.contains(candidate.getType()) && !result.contains(candidate)) {
				if (!findConnectedTree(candidate, result)) {
					return false;
				}
			}
		}
